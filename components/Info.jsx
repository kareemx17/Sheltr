...
const Info = () => {
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Determine risk level based on latitude
    ...
    const analyzeHurricanePath = async () => {
      try {
        ...
        // Only set risk level if not already set by lat/lng
        if (!riskLevel) {
  ...
  const getRiskDescription = (level) => {
    switch (level?.toLowerCase()) {
      ...
      case "low":
        return "This is a low-risk hurricane situation. While the impact is expected to be minimal, it's advised to follow safety protocols and be prepared.";
      default:
