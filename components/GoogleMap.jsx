 ...
 useEffect(() => {
   window.initMap = async function () {
     try {
      ...
      // // { lat: 24.0, lng: -88.0 }, // Gulf of Mexico (south of current position)
      // // { lat: 24.5, lng: -85.0 }, // Moving east
      // // { lat: 25.0, lng: -82.0 }, // South of Florida Keys
      // // { lat: 25.5, lng: -79.5 }, // Passing south of Miami
      // // { lat: 26.0, lng: -77.0 }, // Entering Atlantic
      // // { lat: 26.5, lng: -74.5 }, // Moving northeast
      // // { lat: 27.0, lng: -72.0 }, // Further into Atlantic
      // // { lat: 27.5, lng: -69.5 }
      //           ];
           ...
           const moveHurricane = () => {
             if (currentSegment < pathPoints.length - 1) {
               ...
               arrows.forEach((arrow, index) => {
                 const angle = ((index * 360 / arrows.length) + (progress * 720)) * (Math.PI / 180);
                 const arrowLength = 30000 + (currentSegment * 5000); // Shorter arrows
                 const endLat = newPosition.lat + (arrowLength * Math.sin(angle) / 111000);
                 const endLng = newPosition.lng + (arrowLength * Math.cos(angle) / (111000 * Math.cos(newPosition.lat * Math.PI / 180)));
           ...
           for (let i = 0; i < numArrows; i++) {
             ...
             const endLat = startLat + (arrowLength * Math.sin(angle) / 111000);
             const endLng = startLng + (arrowLength * Math.cos(angle) / (111000 * Math.cos(startLat * Math.PI / 180)));
             ...
             const impactInfoWindow = new window.google.maps.InfoWindow({
               content: `
                 <div style="padding: 10px; max-width: 250px;">
                   <h3 style="margin: 0 0 5px 0; color: #FF0000;">Impact Zone ${index + 1}</h3>
                   <p style="margin: 0 0 5px 0;">Wind Speed: ${120 + (index * 5)} mph</p>
                   <p style="margin: 0;">Expected Time: ${24 - (index * 3)} hours</p>
                 </div>
               `,
               position: point
             });
